...
/*Important: input assumes that the time is provided in the GMT time zone*/
time_t fmktime_(int * yy, int * mm, int * dd, int * hh, int * min, int * ss){
    ...
    //the following 3 values are not needed (will be recomputed in mktime)
...
//function to format a time in a string according to the C function strftime
